...
if __name__=='__main__':
    ...
    if loss_dir.endswith('/'):
        loss_dir = loss_dir[:-1]
    np.save(f"{loss_dir}/more_dropout_{batch_size}_{input_size}_{int(time.time())}.npy", np.array(losses))  # type: ignore
    ...
    checkpoint_location = f"{checkpoint_dir}/more_dropout"
